...
export default function Edit (products) {
  const {data, setData, processing, reset, errors} = useForm ({
    ...
    stock_quantity: products.products.stock_quantity,
    size_ids: products.products.size_ids,
  ...
  const submit = (e) => {
    ...
    if (data.image) { // Only append image if it exists
    ...
    router.post(route('product.update', products.products.id), formData, { 
      onSuccess: () => reset(),
    ...
    <div>
      ...
      <form onSubmit={submit} className='flex items-center flex-col flex-wrap gap-4'>
          <div className="flex flex-col pl-4 max-w-[300px] gap-x-4">
            ...
            >
              {Array.isArray(products.sizes) && products.sizes.map((size) => {
                return(
